...
const showOtherSections = (e) => {
  ...
  const priceInput = $priceFormInput.value;
...
const displayLottoSection = (priceInput) => {
  ...
  // 2. <ul> 구매 갯수만큼 티켓 생성
  for (let i = 1; i <= AMOUNT_OF_LOTTOS; i++) {
    // 티켓 <li> 생성
    $lottoTicketsUl.innerHTML = template;

    // 중복되지 않은 랜덤 넘버 6개 생성
    while (randomNums.size !== AMOUNT_OF_LOTTOS * 6) {
      randomNums.add(
        Math.floor(
    ...
    for (let n of randomNums) {
      $lottoTicketNumSpan.innerText += n;
    }
  }
};
