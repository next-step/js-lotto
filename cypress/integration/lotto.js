...
describe('로또 어플리케이션 테스트', () => {
  ...
  describe('로또 1장의 가격은 1,000원이다.', () => {
    ...
    it('금액이 최대 금액을 넘을경우 alert을 띄워준다.', () => {
  ...
  describe('다시 시작하기 버튼을 누르면 초기화 되서 다시 구매를 시작할 수 있다.', () => {
    ...
    it('다시 시작하기 버튼이 있고 누르면 모달이 사라진다.', () => {
      cy.getDataset('reset-button').should('exist');
      cy.getDataset('reset-button').click();
      cy.getDataset('result-modal').should('not.exist');
    });
