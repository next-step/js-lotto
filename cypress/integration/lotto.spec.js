...
describe('로또 테스트 입니다.', () => {
  ...
  it('로또는 장당 1000원 입니다.', () => {
    cy.get('input[placeholder="구입 금액"]').type(1000)
    cy.get('button#lottoPriceButton').click()
  ...
  it('로또의 번호는 1 ~ 45까지 숫자이고 6개의 배열 값을 반환한다.', () => {
    ...
    const isValidateLotto = (lottoNumbers) =>
      lottoNumbers.filter((item, index) => lottoNumbers.indexOf(item) != index)
        .length === 0
  ...
  it('로또 번호보기 토글 클릭 시 로또 번호는 보입니다.', () => {
    ...
    cy.get('button#lottoPriceButton').click()
    cy.get('.switch').click()
    cy.get('span[name*=lottoNumbers]').each((item, index, list) => {
      expect(list).to.have.css('display', 'inline-block')
    })
  })
})
