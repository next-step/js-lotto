...
function inputSpec() {
    ...
    it('1000원 이하의 금액을 입력하고 확인 버튼을 누르면 최소금액 미달 경고창이 뜬다.', () => {
        ...
        checkAlert(clickPurchaseButton(), '1000원 이하로 구입할 수 없습니다.');
    })

    it('10000원 이상의 금액을 입력하고 확인 버튼을 누르면 최대금액 초과 경고창이 뜬다.', () => {
        typePurchaseInput('11000');
        checkAlert(clickPurchaseButton(), '10000원 이상 구입할 수 없습니다.');
    })
...
function purchaseSpec() {
    ...
    it('1000 단위의 금액을 입력하고 Enter 키를 누르면 로또 티켓이 금액에 맞는 갯수만큼 발급된다.', () => {
        typePurchaseInput('3000').type('{enter}');
