const { LOTTO, ERROR_MESSAGE } = require('../../src/js/constants/index.js');
const { SELECTOR } = require('../constants/selector.js');
const { generateRandomNumbersToArray } = require('../../src/js/utils/index.js');
...
describe('로또 어플리케이션을 테스트한다.', () => {
  ...
  it('로또 구입 금액을 1000원 단위로 입력하지 않고 키보드의 엔터키를 누르면 사용자에게 alert를 띄워준다.', () => {
    ...
    cy.getByDataset(SELECTOR.PURCHASE_PRICE_INPUT)
      .type('1500{enter}')
      .then(() => {
  ...
  it('로또 구입 금액을 1000원 단위로 입력하지 않고 확인 버튼을 클릭하면 사용자에게 alert를 띄워준다.', () => {
    ...
    cy.getByDataset(SELECTOR.PURCHASE_BUTTON)
      .click()
      .then(() => {
        expect(alertStub.getCall(0)).to.be.calledWith(ERROR_MESSAGE.INVALID_LOTTO_PRICE_UNIT);
      });
