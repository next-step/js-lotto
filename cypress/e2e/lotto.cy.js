import { LOTTO_NUMBER } from '../../src/js/service/Constant';
import { generateLottoNumbers } from '../../src/js/service/LottoBuyer';
...
describe('로또 난수 생성 로직을 점검한다', () => {
  ...
  it('로또가 6자리 생성되는지 검사한다', () => {
    ...
    expect(lottoNumbers.length).to.eq(LOTTO_NUMBER.LENGTH);
  });

  it('로또 난수가 중복없이 생성되는지 검사한다', () => {
    const lottoNumbers = generateLottoNumbers();
    const deduplicatedLottoNumbers = Array.from(new Set(lottoNumbers));
    expect(deduplicatedLottoNumbers.length).to.eq(LOTTO_NUMBER.LENGTH);
  });
});
