import { inputSelector, buttonSelector, spanSelector, formSelector, liSelector } from '../../src/js/constants/selectors';
...
describe('로또 구매 기능 테스트', () => {
	...
	context('로또 구입 금액 입력 테스트', () => {
		...
		it('구입 금액 입력창에 숫자를 입력할 수 있다.', () => {
			cy.get(inputSelector.LOTTO_PURCHASE_FORM_INPUT).type(1000).should('have.value', 1000);
		});
	});
	...
	context('로또 발급 테스트', () => {
		...
		it('구입 금액 입력창에 1000을 입력하고 확인 버튼을 누르면 로또가 1장 발급된다.', () => {
			cy.submitPriceForm(1000).then(() => {
				cy.get(spanSelector.PURCHASED_LOTTO_COUNT_TEXT).should('have.text', '1');
			});
		});

		it('구입 금액 입력창에 30000을 입력하고 확인 버튼을 누르면 로또가 30장 발급된다.', () => {
			cy.submitPriceForm(30000).then(() => {
				cy.get(spanSelector.PURCHASED_LOTTO_COUNT_TEXT).should('have.text', '30');
			});
		});
	});
	...
	context('로또 번호 visible 테스트', () => {
		it('로또를 구매하면, 번호보기 토글 버튼이 화면에 보인다', () => {
			cy.submitPriceForm(10000).then(() => {
