...
describe("Lotto Result Class Test .", () => {
  ...
  it("N개의 로또 티켓을 비교하여 {ticket - origin ticket값을, match - 일치하는 값을 갖고있는 배열, hasBonus - 보너스 번호 일치 여부}를 배열로 return 한다.", () => {
    ...
    const result = lottoResult.compareNumber();
  ...
  it("당첨 번호가 1미만, 45초과된다면 INVALID_WINNING_NUMBER_RANGE 에러를 throw 한다.", () => {
    ...
    expect(
      () =>
        new LottoResult({
          winningNumbers: errorNumbers,
          bonusNumber: bonusNumber,
          tickets: ticket,
        })
    ).toThrow(LOTTO_RESULT_ERROR_MESSAGE.INVALID_WINNING_NUMBER_RANGE);
  });
  ...
  it("당첨 번호가 1미만, 45초과 시 INVALID_BONUS_NUMBER_RANGE 에러를 throw 한다.", () => {
    ...
    expect(
      () =>
        new LottoResult({
          winningNumbers: winningNumbers,
          bonusNumber: errorBonusNumber,
          tickets: ticket,
        })
    ).toThrow(LOTTO_RESULT_ERROR_MESSAGE.INVALID_BONUS_NUMBER_RANGE);
  });
  ...
  it("당첨 번호가 6자리 미만 이거나, 초과 된다면 INVALID_WINNING_NUMBERS_LENGTH를 throw 한다.", () => {
    ...
    ).toThrow(LOTTO_RESULT_ERROR_MESSAGE.INVALID_WINNING_NUMBERS_LENGTH);
  });

  it("보너스 번호가 입력되지 않으면 BONUS_NUMBER_REQUIRED를 throw 한다.", () => {
    expect(() => new LottoResult({ winningNumbers: winningNumbers })).toThrow(
  ...
  it("당첨번호와 보너스숫자에 중복이 있으면 DUPLICATE_NUMBERS_NOT_ALLOWED를 throw 한다.", () => {
    ...
    ).toThrow(LOTTO_RESULT_ERROR_MESSAGE.DUPLICATE_NUMBERS_NOT_ALLOWED);
  });
