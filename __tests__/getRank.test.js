...
describe("사용자가 6개의 당첨 번호와 1개의 보너스 번호를 입력한 후", () => {
  ...
  ])("당첨 번호와 보너스 번호가 올림 차순 배열과 보너스 번호를 가진 하나의 배열로 반환된다.", (userNumbers, bouns, expected) => {
    expect(transformUserInput(userNumbers, bouns)).toEqual(expected);
  });
  ...
  test.each([
    ...
    [[1, 2, 3, 8, 9, 10], 5],
    [[11,12,13,14,15,16], 6],
  ])('당첨 번호와 로또 번호가 일치하는 수에 따라서 등수가 반환된다.', (lotto, expected) => {
  ...
  test.each([
    ...
    [4, 50_000],
    [5, 5_000],
  ])("등수에 따른 금액이 지급된다.", (rank, expectedReward) => {
    //1등 2,000,000,000
    //2등 30,000,000
    //3등 1,500,000
    //4등 50,000
