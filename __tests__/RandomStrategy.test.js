...
describe("RandomIssueStrategy	테스트", () => {
  describe("_getNumber() 테스트", () => {
    it("lowerBound, upperBound 없이 생성하면, [1, 45] 사이 숫자를 반환한다.", () => {
      const randomIssueStrategy = new RandomIssueStrategy();
      expect(randomIssueStrategy._getNumber()).toBeGreaterThanOrEqual(1);
      expect(randomIssueStrategy._getNumber()).toBeLessThanOrEqual(45);
    });
    ...
    it("lowerBound, upperBound가 주어진 경우, [lowerBound, upperBound] 사이 숫자를 반환한다.", () => {
      ...
      expect(randomIssueStrategy._getNumber()).toBeGreaterThanOrEqual(1);
      expect(randomIssueStrategy._getNumber()).toBeLessThanOrEqual(100);
    });
  });
